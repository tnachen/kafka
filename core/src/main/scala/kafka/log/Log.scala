...
class Log(val dir: File,
  ...
  private def kafkaStorageCheck[T](message: => String)(fun: => T): T = {
    try {
      fun
    } catch {
      case e: IOException =>
        throw new GenericKafkaStorageException(this.dir, message, e)
    }
  }

  /* Load the log segments from the log files on disk */
  private def loadSegments() {
    kafkaStorageCheck("Failed to load segments from disk") {
  ...
  def append(messages: ByteBufferMessageSet, assignOffsets: Boolean = true): LogAppendInfo = {
    ...
    lock synchronized {
      kafkaStorageCheck("I/O exception in append to log '%s'".format(name)) {
        appendInfo.firstOffset = nextOffsetMetadata.messageOffset

        if(assignOffsets) {
          // assign offsets to the message set
          val offset = new AtomicLong(nextOffsetMetadata.messageOffset)
          kafkaStorageCheck("Error in validating messages while appending to log '%s'".format(name)) {
            validMessages =
              validMessages.validateMessagesAndAssignOffsets(
                offset, appendInfo.sourceCodec, appendInfo.targetCodec, config.compact)
          }
  ...
  def flush(offset: Long) : Unit = {
    ...
    kafkaStorageCheck("Failed to flush the log") {
  ...
  private[log] def delete() {
    lock synchronized {
      kafkaStorageCheck("Failed to delete log") {
        removeLogMetrics()
  ...
  private[log] def truncateTo(targetOffset: Long) {
    ...
    lock synchronized {
      kafkaStorageCheck("Failed to truncate log") {
  ...
  private[log] def truncateFullyAndStartAt(newOffset: Long) {
    ...
    lock synchronized {
      kafkaStorageCheck("Failed to fully truncate log") {
  ...
  private[log] def replaceSegments(newSegment: LogSegment, oldSegments: Seq[LogSegment], isRecoveredSwapFile : Boolean = false) {
    lock synchronized {
      kafkaStorageCheck("Failed to replace old segments") {
